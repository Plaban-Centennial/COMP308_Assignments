...
exports.addGame = async function (req, res) {
	...
	console.log('in addGame:', payload)

	try {

		await User.findOneAndUpdate(
			{ _id: payload.id }, // Find the document by its `_id`
			{ $addToSet: { games: req.game._id } }, // Add `newId` if it's not already in `listField`
			{ new: true }, // Return the updated document
		);

		res.status(200).send({ screen: 'view' });
...
exports.deleteGame = async function (req, res) {
	...
	console.log('in deleteGame:', payload)

	try {

		await User.findOneAndUpdate(
			{ _id: payload.id }, // Find the document by its `_id`
			{ $pull: { games: req.game._id } }, // Add `newId` if it's not already in `listField`
			{ new: true }, // Return the updated document
		);
